...
def get_filters():
    ...
    if cl.confirm("WANT SOME CUSTOM FILTERS FOR {} ?".format(city.upper())):
        while True:
            ...
            month = input(
                'Alright then, enter a month to explore. eg. \"january,february,march, april,may,june\"\n>')
...
def station_stats(df):
    ...
    most_freq_start_end_station = df[[
...
def Show_genderbased_trip_duration(df):
    ...
    if check_column_exists('Gender', df):
        n = df.groupby(['Gender'])['Trip Duration'].sum(
        ).reset_index(name="Trip Duration")
        # check if matplot has been imported successfully
        if import_checker:
            # PLot the graph
            plt.ticklabel_format(style='plain')
            plt.bar(n.loc[:, 'Gender'], n.loc[:, 'Trip Duration'],
...
def Show_according_to_gender_and_the_type(df, gender):
    ...
    if check_column_exists('Gender', df) and check_column_exists('User Type', df):
        group_user_and_gender = df.groupby(
            ['Gender', 'User Type']).size().reset_index(name="Counter")
        ...
        if import_checker:
            # PLot the graph
            plt.ticklabel_format(style='plain')
            plt.plot(gendertype.loc[:, "User Type"],
                     gendertype.loc[:, "Counter"], 'go', label=gender)
            plt.plot(gendertype.loc[:, "User Type"],
                     gendertype.loc[:, "Counter"], color="orange")
...
def display_raw_data_to_user(df):
    ...
    while row < data_length-1:
        if cl.confirm("\nWould you like to see the raw data? Type 'Yes' or 'No'.\n"):
            ...
            result = df.iloc[row:row+5, :].fillna(0).to_json(
                orient="records", lines=True).split('\n')
